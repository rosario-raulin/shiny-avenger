...
make_grouping_algorithm(const std::string& algorithm) {
	if (algorithm == "radix") {
        return std::unique_ptr<IGroupingAlgorithm>(new RadixGrouping(4, 16, 11));
	} else if (algorithm == "simple") {
        return std::unique_ptr<IGroupingAlgorithm>(new HashBasedGrouping());
	} else if (algorithm == "shared") {
        return std::unique_ptr<IGroupingAlgorithm>(new SharedHashGrouping());
	} else {
		return nullptr;
	}
}
...
testGroupingAlgorithm(std::unique_ptr<IGroupingAlgorithm> algo, std::size_t testsize, std::size_t cardinality) {
	Column column = make_column(testsize, cardinality);
